...
def divide_data(dataset):
  # five fold cross validation
    ...
    sets = np.array_split(dataset_arr, 5)
...
def find_accuracy(test, MEAN_1, MEAN_2, STD_1, STD_2, train_positive_gender_prob, train_negative_gender_prob, cnt_is_patient, cnt_is_not_patient):
    ...
    for row in test_positive.iterrows():
        ...
        ret = predict(row[1], column_headers, MEAN_1, STD_1, MEAN_2, STD_2, train_positive_gender_prob,
                      train_negative_gender_prob, cnt_is_patient, cnt_is_not_patient)
        if ret == True:
            tp = tp+1
        else:
            fn = fn+1
...
def main():
    ...
    print('Accuracy of training by 70:30 random split')
    ...
    df_set = divide_data(dataset_filtered)
    ...
    print('Accuracy result of each iteration of 5 fold cross validation')
    ...
    print('Average accuracy of 5 fold cross validation is', np.mean(acc))
    ...
    print('Accuracy of training after Laplace correction')
    print(find_accuracy(test, MEAN_1_laplace, MEAN_2_laplace, STD_1_laplace, STD_2_laplace, train_positive_gender_prob_laplace,
                        train_negative_gender_prob_laplace, prior_is_patient_laplace, prior_is_not_patient_laplace))
