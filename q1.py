...
ATTRIBUTES = ['cement','slag','flyash','water','superplasticizer','coarseaggregate','fineaggregate','age']
...
class RegressionTree:
        ...
        if len(db) <= LIMIT_SIZE or depth >= MAX_DEPTH:
            ...
            for atrribute in ATTRIBUTES:
                ...
                for i in range(len(new_db)):
                    ...
                    mean2 = right_db.mean()
                    cur_sse = 0
                    cur_sse += sum([(left_db.iloc[j]-mean1)**2 for j in range(len(left_db))])
                    cur_sse += sum([(right_db.iloc[j]-mean2)**2 for j in range(len(right_db))])
            ...
            self.L = RegressionTree(db.loc[db[self.attrb] < self.left_value],depth+1,False,LIMIT_SIZE,MAX_DEPTH)
            self.R = RegressionTree(db.loc[db[self.attrb] >= self.left_value],depth+1,False,LIMIT_SIZE,MAX_DEPTH)
    ...
    def prune(self,test_input:pd.DataFrame,root,y_true:np.ndarray):
        ...
        if(not self.isRoot):
            ...
            if(cur_acc >= prev_acc):
                # print(f"{cur_acc} -> {prev_acc} : Prune")
                return
